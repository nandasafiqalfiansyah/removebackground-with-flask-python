function processImage() {
    ...
    .then(data => {
        ...
        if (data.result === 'success') {
            document.getElementById('rgbValues').innerText = `R: ${data.rgb_values.R.toFixed(2)}, G: ${data.rgb_values.G.toFixed(2)}, B: ${data.rgb_values.B.toFixed(2)}`;
            document.getElementById('originalImage').src = `data:image/png;base64,${data.original_image}`;
            document.getElementById('removedBgImage').src = `data:image/png;base64,${data.image_with_removed_bg}`;
